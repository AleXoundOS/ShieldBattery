...
/**
 * Marks the given notification(s) for a particular user as not visible, i.e. cleared.
 */
export async function clearById(userId: number, notificationIds: string[]): Promise<void> {
  const { client, done } = await db()
  try {
    await client.query(sql`
      UPDATE notifications
